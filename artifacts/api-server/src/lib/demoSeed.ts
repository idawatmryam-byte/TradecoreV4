...
/**
 * Create or REFRESH the demo account: upsert the demo user, then wipe and
 * re-seed all of its data. Idempotent — safe to run repeatedly to keep the
 * demo pristine. Returns the demo user id.
 */
export async function seedDemoAccount(): Promise<number> {
...
/**
 * Ensure a demo account EXISTS, seeding it once if missing. Unlike
 * seedDemoAccount() this never wipes an existing demo — it only creates one
 * when there is none, so it's safe to call on every server startup. Returns
 * whether it created the account. Callers gate this on the demo being enabled.
 */
export async function ensureDemoAccount(): Promise<{ created: boolean; userId: number | null }> {
