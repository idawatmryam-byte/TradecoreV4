...
} from "../lib/autopilot/fingerprints";
import {
  createMandate,
...
const transitionVersionSchema = z
  .object({
    toState: z.enum([
      "RESEARCH",
      "SHADOW",
...
function badRequest(res: Response, error: unknown): void {
  const message =
    error instanceof Error ? error.message : "Request was refused";
  res
    .status(
      /not found/i.test(message)
        ? 404
        : /not allowed|cannot|refused|expired|changed|mismatch/i.test(message)
          ? 409
          : 400,
    )
    .json({ error: message });
...
router.get("/autopilot/control", async (req, res): Promise<void> => {
  ...
  res.json({
    ...
    globalSuspensionReason: platformSafety.reason,
    snapshot,
    versions,
    mandates,
    events,
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const expected =
      parsed.data.toState === "DEMO_APPROVED"
    ...
    if (parsed.data.confirmation !== expected) {
      res.status(400).json({ error: `confirmation must equal ${expected}` });
      return;
    }
    try {
      ...
      if (
        ...
        if (snapshot.mandate?.brainVersionId === id) {
          await setAutopilotState({
            ...
            reason: parsed.data.reason,
            actor: { actorType: "user", actorUserId: req.userId! },
...
router.post("/autopilot/mandates", async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (config.mode !== "autopilot")
      throw new Error(
        "Set this configuration to Autopilot mode before freezing its mandate",
      );
    ...
    if (!DemoAutopilotAuthoritySchema.safeParse(authority).success)
      throw new Error(
        "Autonomous Live authority is categorically disabled in Phase 10",
      );
    const requestedInstruments = [
      ...new Set(parsed.data.instruments.map((item) => item.trim())),
    ].sort();
    const configuredInstruments = new Set(
      config.pairs
        .split(",")
        .map((item) => item.trim())
        .filter(Boolean),
    );
    if (requestedInstruments.some((item) => !configuredInstruments.has(item)))
      throw new Error(
        "Every mandate instrument must be present in the exact bot configuration",
      );
    const strategyConfigs = await loadStrategyConfigs(
      req.userId!,
      req.section!,
    );
    ...
    for (const strategyId of [...new Set(parsed.data.strategyIds)].sort()) {
      ...
      if (!strategyConfig?.enabled)
        throw new Error(`Strategy ${strategyId} is missing or disabled`);
      strategyVersions[strategyId] = strategyConfigVersion(
        strategyId,
        strategyConfig,
      );
    }
    const maximumConfiguredLeverage =
    ...
    const bounds: Array<[number, number, string]> = [
      [
        parsed.data.maximumPositionSizeUsdt,
        maximumConfiguredPositionNotionalUsdt,
        "maximumPositionSizeUsdt",
      ],
      [
        parsed.data.maximumLeverage,
        maximumConfiguredLeverage,
        "maximumLeverage",
      ],
      [
        parsed.data.maximumPortfolioRiskPercent,
        Number(config.maxPortfolioRiskPercent),
        "maximumPortfolioRiskPercent",
      ],
      [
        parsed.data.maximumSymbolExposurePercent,
        Number(config.maxSymbolConcentrationPercent),
        "maximumSymbolExposurePercent",
      ],
      [
        parsed.data.maximumNetExposurePercent,
        Number(config.maxNetExposurePercent),
        "maximumNetExposurePercent",
      ],
      [
        parsed.data.maximumCorrelatedExposurePercent,
        Number(config.maxCorrelatedExposurePercent),
        "maximumCorrelatedExposurePercent",
      ],
      [
        parsed.data.dailyLossLimitUsdt,
        Number(config.dailyLossLimitUsdt),
        "dailyLossLimitUsdt",
      ],
      [
        parsed.data.maximumConcurrentPositions,
    ...
    for (const [requested, configured, name] of bounds) {
      if (requested > configured)
        throw new Error(
          `${name} cannot expand the existing deterministic configuration limit (${configured})`,
        );
    }
    const expiresAt = new Date(parsed.data.expiresAt);
    const now = new Date();
    if (expiresAt.getTime() <= now.getTime())
      throw new Error("Mandate expiry must be in the future");
    if (expiresAt.getTime() > now.getTime() + 90 * 24 * 60 * 60 * 1000)
      throw new Error("Demo Autopilot mandates may not exceed 90 days");
    const versions = await listBrainVersions(req.userId!, req.section!);
    const brain = versions.find(
      (item) => item.id === parsed.data.brainVersionId,
    );
    if (!brain || brain.state !== "DEMO_APPROVED")
      throw new Error("The exact brain version is not DEMO_APPROVED");
    const mandate = await createMandate(
      {
        ...
        executionAuthority: authority as z.infer<
          typeof DemoAutopilotAuthoritySchema
        >,
        marketType: config.marketType as "spot" | "futures" | "forex",
...
router.post("/autopilot/activate", async (req, res): Promise<void> => {
  ...
  try {
    const platformSafety = await getPlatformAutopilotSafetyState();
    if (platformSafety.effectiveSuspended) {
      ...
      throw new Error(
        "Global Demo Autopilot suspension is active; activation is refused",
      );
    }
    ...
    const mandate = (await listMandates(req.userId!, req.section!)).find(
      (item) => item.id === parsed.data.mandateId,
    );
    if (!mandate) throw new Error("Mandate not found");
    if (
      config.mode !== "autopilot" ||
      autopilotConfigFingerprint(config) !== mandate.configFingerprint
    )
      throw new Error(
        "Configuration changed or is not in Autopilot mode; create a new immutable mandate",
      );
    const authority = resolveExecutionAuthority({
      section: req.section!,
      marketType: config.marketType as "spot" | "futures" | "forex",
    ...
    if (
      authority !== mandate.executionAuthority ||
      !DemoAutopilotAuthoritySchema.safeParse(authority).success
    )
      throw new Error(
        "Execution authority does not match the approved Demo/testnet/practice mandate",
      );
...
router.post("/autopilot/resume", async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (platformSafety.effectiveSuspended)
      throw new Error(
        "Global Demo Autopilot suspension is active; resume is refused",
      );
    const config = await getOrCreateEngine(req.userId!, req.section!, {
      resumeIdleDemo: false,
    }).loadConfig();
    if (
      ...
      throw new Error("Configuration changed; a new mandate is required");
    const control = await setAutopilotState({
      userId: req.userId!,
      section: req.section!,
      state: "AUTOPILOT_ENABLED",
      reasonCode: "HUMAN_RESUME_APPROVED",
      reason: parsed.data.reason,
      actor: { actorType: "user", actorUserId: req.userId! },
      mandateId: snapshot.mandate.id,
      configSuspended: false,
