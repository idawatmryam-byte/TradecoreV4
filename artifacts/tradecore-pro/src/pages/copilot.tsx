...
const FILTERS = [
  ...
  { key: "executing,executed,rejected,expired,stale,invalidated,superseded,blocked",
    label: "History",
  },
] as const;
...
function RecommendationCard({ rec }: { rec: Recommendation }) {
  ...
  return (
    <Card
      className={cn(
        "border",
        !actionable && rec.status !== "created" && "opacity-80",
      )}
    >
      <CardContent className="p-4">
        ...
        {rec.status === "created" && (
          <p
            className={cn(
              "mt-3 text-[13px] font-mono", remaining.urgent ? "text-warning font-bold" : "text-muted-foreground",
            )}
          >
        ...
        {rec.resolutionReason && rec.status !== "created" && (
          <p
            className={cn(
              "mt-3 text-[13px] font-mono flex items-start gap-1.5",
          ...
          <div className="mt-4 border-t border-border pt-4">
            <Link href={`/copilot/${rec.id}`}>
              <Button size="sm">
                ...
                Review evidence and approval controls
              </Button>
            </Link>
          </div>
        )}
